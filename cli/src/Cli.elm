            ...
            (Cli.OptionsParser.build CliOptions
                ...
                |> Cli.OptionsParser.with
                    (Cli.Option.flag "no-elm-format"
                        |> Cli.Option.withDescription "Don't run elm-format on the outputs."
                    )
                |> Cli.OptionsParser.with
...
formatOptionDescription : List String -> String
formatOptionDescription descriptionLines =
    descriptionLines
        |> List.indexedMap
            (\i line ->
                if i == 0 then
...
withInnerStep index total label toTask steps =
    let
        fullLabel : String
        fullLabel =
            counter index total ++ " " ++ label

        combinedTask : ( a, c, d ) -> BackendTask FatalError ( b, c, d )
    ...
    List.foldl
        (\input ( index, steps ) ->
            ...
            , steps
                ...
                |> withStep (counter index total ++ " Parse OAS")
